...
This is a modified/fixed version of KinematicCharacterController
*/
public class CharacterController extends ActionInterface {
	...
	private static Vector3f[] upAxisDirection = new Vector3f[] {
		...
		public CharacterController(PairCachingGhostObject ghostObject, ConvexShape convexShape, float stepHeight) {
			this(ghostObject, convexShape, stepHeight, 1);
		}

		public CharacterController(PairCachingGhostObject ghostObject, ConvexShape convexShape, float stepHeight, int upAxis) {
			this.upAxis = upAxis;
		...
		protected boolean recoverFromPenetration(CollisionWorld collisionWorld) {
			...
			if(!canJump() && lastPosY > currentPosition.y && verticalVelocity > 0){
				verticalVelocity = 0;
			}
			lastPosY = currentPosition.y;
